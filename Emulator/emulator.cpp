...
void execute(uint8_t *mem, instr *imem, label_loc *labels, int label_count, bool start_immediate)
{
	...
	while (!dexit)
	{
		...
		{
			uint64_t r = 0;
			uint64_t a = rf[i.a2.reg];
			uint64_t b = rf[i.a3.reg];
			while (b != 0)
			{
				if ((b & 1) != 0)
					r ^= a; /* 執行無進位加法 */
		...
		case CLMULR:
		{
			uint64_t rs1_val = rf[i.a2.reg];
			uint64_t rs2_val = rf[i.a3.reg];
			uint64_t output = 0;

			for (int j = 0; j < 32; j++)
			{
				if ((rs2_val >> j) & 1)
					output = output ^ (rs1_val >> (32 - j - 1));
			}
		...
		break;
		case MAX:
			rf[i.a1.reg] = (uint32_t)(((int32_t)rf[i.a2.reg] > (int32_t)rf[i.a3.reg])? (int32_t)rf[i.a2.reg]:(int32_t)rf[i.a3.reg]);
			break;
		case MAXU:
		...
		case MIN:
			...
			break;
		case MINU:
			rf[i.a1.reg] = rf[i.a2.reg] > rf[i.a3.reg] ? rf[i.a3.reg] : rf[i.a2.reg];
			break;
		case ORCB:
			rf[i.a1.reg]=0;
			if(rf[i.a2.reg]&0xff000000){
				rf[i.a1.reg]+=0xff000000;
			}
			if(rf[i.a2.reg]&0x00ff0000){
				rf[i.a1.reg]+=0x00ff0000;
			}
			if(rf[i.a2.reg]&0x0000ff00){
				rf[i.a1.reg]+=0x0000ff00;
			}
			if(rf[i.a2.reg]&0x000000ff){
				rf[i.a1.reg]+=0x000000ff;
			}
			break;
		case ORN:rf[i.a1.reg] =(rf[i.a2.reg] | (~rf[i.a3.reg]));
			break;
		case REV8:
		{	(rf[i.a1.reg])=(rf[i.a2.reg])>>24;
			(rf[i.a1.reg])=(rf[i.a1.reg])+((rf[i.a2.reg]>>16)<<8)-((rf[i.a2.reg]>>24)<<16);
			(rf[i.a1.reg])=(rf[i.a1.reg])+((rf[i.a2.reg]>>8)<<16)-((rf[i.a2.reg]>>16)<<24);
			(rf[i.a1.reg])=(rf[i.a1.reg])+(rf[i.a2.reg]<<24);
			break;
		}
		case ROL:
		{
			uint32_t index = rf[i.a3.reg] & 31;
			rf[i.a1.reg] = (rf[i.a2.reg] << index) | (rf[i.a2.reg] >> (32 - index));
		}
		break;
		case ROR:
		{
		...
		{	uint32_t index = rf[i.a3.reg] & 31;
			...
			break;}	
		case BCLRI:
			rf[i.a1.reg] = (rf[i.a2.reg]& ~(1 << (i.a3.imm & 31))) ;
			break;
		case BEXT:
		{
			uint32_t index = rf[i.a3.reg] & 31;
		...
		break;
		case BEXTI:
			rf[i.a1.reg] = (rf[i.a2.reg] >> (i.a3.imm & 31)) & 1;
			break;
		case BINV:
			rf[i.a1.reg] = rf[i.a2.reg] ^ (1 << (rf[i.a3.reg] & 31));
			break;
		case BINVI:
			rf[i.a1.reg] = rf[i.a2.reg] ^ (1 << (i.a3.imm & 31));
			break;
		...
		case BSETI:
			...
			break;
		case SEXTB:
			rf[i.a1.reg] = (int32_t)(int8_t)(rf[i.a2.reg] & 0x000000FF);
			break;
		case SEXTH:
			rf[i.a1.reg] = (int32_t)(int16_t)(rf[i.a2.reg] & 0x0000FFFF);
			break;
		case SH1ADD:
			rf[i.a1.reg] = rf[i.a2.reg] + (rf[i.a3.reg] << 1);
			break;
		case SH2ADD:
			rf[i.a1.reg] = rf[i.a2.reg] + (rf[i.a3.reg] << 2);
			break;
		case SH3ADD:
			rf[i.a1.reg] = rf[i.a2.reg] + (rf[i.a3.reg] << 3);
			break;
		case XNOR:
			rf[i.a1.reg] = ~(rf[i.a2.reg] ^ rf[i.a3.reg]);
			break;
