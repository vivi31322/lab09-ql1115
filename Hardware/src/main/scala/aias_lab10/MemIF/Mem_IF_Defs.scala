...
class MemIF_CPU(val addrWidth: Int) extends Bundle {
  ...
  override def clone = { new MemIF_CPU(addrWidth).asInstanceOf[this.type] }
}

class MemIF_MEM(val addrWidth: Int) extends Bundle {
  ...
  override def clone = { new MemIF_MEM(addrWidth).asInstanceOf[this.type] }
}

class MemIF extends MultiIOModule {
  val io = IO(new Bundle {
    val cpu = new MemIF_CPU(15)
    val mem = new MemIF_MEM(15)
  })
