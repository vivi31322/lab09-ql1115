...
class ALU extends Module {
  ...
  switch(io.ALUSel) {
    ...
    is(CLZ)   { when(io.src1(31) === 1.U){io.out := 0.U}
    ...
    is(CTZ)   { when(io.src1(0) === 1.U){io.out := 0.U}
                .elsewhen(io.src1 === 0.U){io.out := 32.U}
                .otherwise{
                  for(i <- 1 until 32){
    ...
    is(CPOP)  { io.out := 0.U(6.W)##io.src1(0)  + 0.U(6.W)##io.src1(1)   + 0.U(6.W)##io.src1(2) + 0.U(6.W)##io.src1(3) +
    ...
    is(ORC_B) { io.out := Cat(Mux(io.src1(31,24)===0.U, "b00000000".U, "b11111111".U),
