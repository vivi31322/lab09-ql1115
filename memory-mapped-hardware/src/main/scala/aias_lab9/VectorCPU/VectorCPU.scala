...
class VectorCPU(memAddrWidth: Int, memDataWidth: Int, instrBinaryFile: String) extends Module {
  ...
  rf.io.waddr := rd
  rf.io.wdata := MuxLookup(ct.io.WBSel, 0.U, Seq(
      0.U -> MuxLookup(funct3, io.bus_master.readData.bits.data(31, 0), Seq(
  ...
  vrf.io.vector_wdata := MuxLookup(ct.io.vector_WBSel, 0.U, Seq(
  ...
  alu.io.src2 := MuxLookup(ct.io.BSel, 0.U, Seq(
    0.U -> rf.io.rdata(1),
    1.U -> ig.io.imm,
    2.U -> 0.U
  ))
  ...
  io.bus_master.writeData.bits.data := MuxLookup(ct.io.MemSel, 0.U, Seq(
    0.U -> rf.io.rdata(1),
    1.U -> vrf.io.vector_rdata(0)
  ))
  io.bus_master.writeData.bits.strb := MuxLookup(opcode, 0.U, Seq(
    STORE -> (MuxLookup(funct3, 0.U, Seq(
        "b000".U(3.W) -> "b1".U,
        "b001".U(3.W) -> "b11".U,
        "b010".U(3.W) -> "b1111".U
      ))),
